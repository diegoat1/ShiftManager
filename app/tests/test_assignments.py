...
@pytest.mark.asyncio
async def test_check_eligibility_endpoint(client):
    ...
    # Check eligibility
    resp = await client.get(f"/api/v1/assignments/check/{doc_id}/{shift_id}")
...
@pytest.mark.asyncio
async def test_assign_and_unassign(client):
    # Setup minimal
    doc = await client.post("/api/v1/doctors/", json={
    ...
    resp = await client.get(f"/api/v1/assignments/shift/{shift_id}")
    assert resp.status_code == 200
    assert len(resp.json()) == 1

    # Unassign
    resp = await client.delete(f"/api/v1/assignments/{assignment_id}")
    assert resp.status_code == 204


@pytest.mark.asyncio
async def test_assign_ineligible_doctor(client):
    doc = await client.post("/api/v1/doctors/", json={
    ...
    inst_id = inst.json()["id"]
    site = await client.post(f"/api/v1/institutions/{inst_id}/sites", json={"name": "PS"})
    site_id = site.json()["id"]
...
@pytest.mark.asyncio
async def test_get_eligible_doctors(client):
    inst = await client.post("/api/v1/institutions/", json={
    ...
    resp = await client.get(f"/api/v1/assignments/eligible/{shift_id}")
    assert resp.status_code == 200
