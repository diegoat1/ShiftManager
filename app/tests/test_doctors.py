...
async def test_update_doctor(client):
    ...
    doc_id = create.json()["id"]
    resp = await client.patch(f"/api/v1/doctors/{doc_id}", json={"first_name": "Updated"})
    assert resp.status_code == 200
...
async def test_delete_doctor(client):
    ...
    doc_id = create.json()["id"]
    resp = await client.delete(f"/api/v1/doctors/{doc_id}")
...
@pytest.mark.asyncio
async def test_add_certification(client):
    # Create lookup
