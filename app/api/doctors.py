...
from app.api.deps import get_doctor_service
...
async def create_doctor(data: DoctorCreate, svc: DoctorSvc):
    return await svc.create(data)


@router.get("/", response_model=PaginatedResponse[DoctorBrief])
async def list_doctors(svc: DoctorSvc, skip: int = 0, limit: int = 50):
    items, total = await svc.get_all(skip=skip, limit=limit)
    return PaginatedResponse(items=items, total=total, skip=skip, limit=limit)


@router.get("/{doctor_id}", response_model=DoctorRead)
async def get_doctor(doctor_id: uuid.UUID, svc: DoctorSvc):
...
async def update_doctor(doctor_id: uuid.UUID, data: DoctorUpdate, svc: DoctorSvc):
